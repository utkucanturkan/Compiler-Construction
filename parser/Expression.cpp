...
Expression::Expression(std::shared_ptr<const SimpleExpression> _lhs, TokenType _op, std::shared_ptr<const SimpleExpression> _rhs) : lhs(_lhs), op(_op), rhs(_rhs)
{
	if (_lhs.get()->type == _rhs.get()->type)
	{
		type = _lhs.get()->type;
	}
}
